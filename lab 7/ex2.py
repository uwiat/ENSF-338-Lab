...
def insert(data, root=None):
    ...
    if pivot is None:
        ...
        # Check if the node is being inserted into the shorter subtree
        if height(pivot.left) > height(pivot.right) and data > pivot.data:
            print("Case #2: A pivot exists, and a node was added to the shorter subtree")
        elif height(pivot.right) > height(pivot.left) and data < pivot.data:
            print("Case #2: A pivot exists, and a node was added to the shorter subtree")
        else:
            print(f"Pivot detected at node {pivot.data}")
